...
def create_note(
    ...
    db_note = NoteModel(title=note.title, content=note.content, user_id=current_user.id)
    ...
    db_note = NoteModel(title=note.title, content=text_note, user_id=current_user.id)
    db.add(db_note)
    db.commit()
    db.refresh(db_note)
    return db_note
